...
public class LottieCompositionFactory {
    ...
    public static LottieTask<LottieComposition> fromZipStream(Activity context, final ZipInputStream inputStream, @Nullable final String cacheKey) {
     ...
     */
    @WorkerThread
    public static LottieResult<LottieComposition> fromZipStreamSync(Context context, ZipInputStream inputStream, @Nullable String cacheKey) {
        try {
    ...
    private static LottieResult<LottieComposition> fromZipStreamSyncInternal(Context context, ZipInputStream inputStream, @Nullable String cacheKey) {
        ...
        try {
            ...
            while (entry != null) {
                ...
                Log.d("Lottienamesss","Lottienamesss:"+entryName);

                if (entryName.contains("__MACOSX")) {
                    inputStream.closeEntry();
                } else if (entry.getName().equalsIgnoreCase("manifest.json")) {
                    inputStream.closeEntry();
                } else if (entry.getName().contains("template.json")) {
                    inputStream.closeEntry();
                }else if (entry.getName().contains("text.txt")) {
                    inputStream.closeEntry();
                }else if (entry.getName().contains("demo")) {
                    inputStream.closeEntry();
                } else if (entry.getName().contains("data.html")) {
                    inputStream.closeEntry();
                } else if (entry.getName().contains(".json")) {

                    String result1 = IOUtils.toString(inputStream, StandardCharsets.UTF_8);

                    InputStream in = IOUtils.toInputStream(result1, "UTF-8");

                    JsonReader reader = JsonReader.of(buffer(source(in)));
